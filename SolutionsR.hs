...
showSolution "B" =
    let b = solutionB
    in  "B. P(d6+d12) = " ++ (show (100 * (fromRational b))) ++ "%\n"
showSolution "C" = 
    let params = weightedDist [(6,9/46),(8,9/46), (12,14/46), (20,14/46)]
        c1 = (solutionC1 params)
        c2 = (solutionC2 params)
        p1= "C.1. P(draw d6 and d12) = " ++ (show (100 * (fromRational c1))) ++"%\n"
        p2= "C.1. P(exactly one d20) = " ++ (show (100 * (fromRational c2))) ++"%\n"
    in p1 ++ p2
showSolution "D" =
    let params = weightedDist [(6,9/46),(8,9/46), (12,14/46), (20,14/46)]
        d = solutionD params
...
showSolution "G" = "G. Throw d6; toss that many coins; probability of at least 3 heads = "
                    ++ (show (100*(fromRational solutionG))) ++ "%\n"
showSolution "I" = "I. Throw d6; toss that many coins; see 3 heads; P(N=3) = "
                    ++ (show (100*(fromRational solutionI))) ++ "%\n"
showSolution "J" = "J. Throw d6; toss that many coins; N > 4; P(heads=3) = "
                    ++ (show (100*(fromRational solutionJ))) ++ "%\n"
showSolution "K" = "K. (identical to I) ="
                    ++ (show (100*(fromRational solutionK))) ++ "%\n"
showSolution "N" =
    let params = weightedDist [(6,9/46),(8,9/46), (12,14/46), (20,14/46)]
    in "N. P(draw d12+d12 and put 3 marks in right) = "
        ++ (show (100*(fromRational (solutionN params)))) ++ "%\n"
